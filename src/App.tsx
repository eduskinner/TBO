...
function MainApp() {
  ...
  const init = React.useCallback(async () => {
    ...
    const tryInit = async () => {
      ...
      try {
        await loadLibrary();
        // ── Show the UI immediately after the library data arrives ──────────
        setReady(true);

        // ── Fire background tasks — none of these block the UI ──────────────
        // 1. Load sources (sidebar panel, not needed to render the library)
        loadSources().catch(() => {});
      } catch (e: any) {

...
export default function Settings() {
  const { sources, loadSources, removeSource, openAddFolder, clearMissingComics } = useStore();
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 max-w-2xl">
        <Section title="Library Sources">
          <p style={{ fontSize: 13, color: "var(--text2)", marginBottom: 16, lineHeight: 1.6 }}>
            Lector TBO scans these folders for CBZ and CBR files. Adding a folder again will skip already-catalogued files.
          </p>
        ...
        <Section title="CBR Support">
          ...
          >
            <p>CBZ files work out of the box. For CBR (RAR) files, Lector TBO uses your system's <code style={{ color: "var(--accent)", fontFamily: "monospace" }}>bsdtar</code> or <code style={{ color: "var(--accent)", fontFamily: "monospace" }}>unrar</code>.</p>
        ...
        <Section title="About">
          <div className="text-sm" style={{ color: "var(--text2)", lineHeight: 2 }}>
            <p><span style={{ color: "var(--text)" }}>Lector TBO</span> v0.1.0</p>
            <p>A comic reader and library manager built with Tauri + React.</p>
            <p style={{ marginTop: 8, color: "var(--text3)", fontSize: 12 }}>
              Database stored at: <code style={{ fontFamily: "monospace", color: "var(--text2)" }}>~/.local/share/lector-tbo/panels.db</code>
            </p>
            <button
              onClick={async () => {
                if (confirm("Remove all 'Missing' entries from your library?")) {
                  await clearMissingComics();
                }
              }}
              className="mt-6 px-4 py-2 rounded-lg text-xs font-semibold transition-all border border-red-500/30 text-red-500/80 hover:bg-red-500 hover:text-white"
            >
              Clear Missing Comics
            </button>
          </div>
        </Section>
      </div>
